...
// jQuery Artscore Studio Framework Search Identity Plugin for Symfony2
// A jQuery Plugin used in Symfony2 project for interactive search identity field
//
// version 0.1, 2015-09-21
// by Nicolas Claverie <info@artscore-studio.fr>
(function($){
    $.asfSearchIdentity = function(element, options) {
        
        // Plugin's default options
        // Private property accesible only from inside the plugin
        var defaults = {
            identityRequestRoute: 'asf_contact_ajax_request_identity_by_name',
            identitySuggestRoute: 'asf_contact_ajax_request_identity_suggest',
            modalId: '#search-identity-modal'
        }
        ...
        plugin.autocomplete = {
            init: function(){
                $element.select2({
                    ajax: {
                        url: Routing.generate(plugin.settings.identityRequestRoute),
                        dataType: 'json',
                        delay: 250,
        ...
        plugin.onChange = function(event, ui) {
            $modal.find('.modal-title').text(Translator.trans('asf.contact.form.search_identity.modal.title'));
            $modal.modal('show');
            $.ajax({
                url: Routing.generate(plugin.settings.identitySuggestRoute, {term: $element.val()}),
                success: plugin.suggestedIdentityWindow
            });
        }

        plugin.suggestedIdentityWindow = function(response){
            ...
            $modal.find('.btn-submit').on('click', function(){
                ...
                $(this).addClass('btn-waiting').text(Translator.trans('asf.contact.form.search_identity.modal.loading'));
